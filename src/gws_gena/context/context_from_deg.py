    ...
    config_specs = ConfigSpecs(
        {
            ...
            "log2_fold_change_column": StrParam(
                default_value="avg_log2FC", human_name="Log2 fold change column name"
            ),
            "threshold": FloatParam(
                default_value=0.25,
                human_name="Threshold for log fold change",
                visibility=StrParam.PROTECTED_VISIBILITY,
            ),
            "target_high": FloatParam(
                default_value=1, human_name="Target high", visibility=StrParam.PROTECTED_VISIBILITY
            ),
            "lower_bound_high": FloatParam(
                default_value=0.5,
                human_name="Lower bound high",
                visibility=StrParam.PROTECTED_VISIBILITY,
            ),
            "upper_bound_high": FloatParam(
                default_value=1000,
                human_name="Upper bound high",
                visibility=StrParam.PROTECTED_VISIBILITY,
            ),
            "target_low": FloatParam(
                default_value=0, human_name="Target low", visibility=StrParam.PROTECTED_VISIBILITY
            ),
            "lower_bound_low": FloatParam(
                default_value=0,
                human_name="Lower bound low",
                visibility=StrParam.PROTECTED_VISIBILITY,
            ),
            "upper_bound_low": FloatParam(
                default_value=0,
                human_name="Upper bound low",
                visibility=StrParam.PROTECTED_VISIBILITY,
            ),
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        for reaction in network_dict["reactions"]:
            if reaction["gene_reaction_rule"]:
                ...
                value = self.evaluate_gene_reaction_rule(
                    reaction["gene_reaction_rule"], df_deg, gene_id_column, log2_fold_change_column
                )
                ...
                if value is not None:
                    if value < -threshold:
                        ...
                        row2 = pd.DataFrame(
                            [
                                [
                                    reaction["id"],
                                    target_condition2,
                                    lower_bound_condition2,
                                    upper_bound_condition2,
                                    confidence,
                                ]
                            ],
                            columns=columns_context_flux_table,
                        )
                        context_flux_table = pd.concat([context_flux_table, row2])

                        row_reactions_modified = pd.DataFrame(
                            [[reaction["id"], "low"]], columns=("Reaction", "Flux traceability")
                        )
                        list_reactions_modified = pd.concat(
                            [list_reactions_modified, row_reactions_modified]
                        )
                        ...
                        row_reactions_modified = pd.DataFrame(
                            [[reaction["id"], "high"]], columns=("Reaction", "Flux traceability")
                        )
                        list_reactions_modified = pd.concat(
                            [list_reactions_modified, row_reactions_modified]
                        )
        ...
        # return the output
        return {
            "context_flux_table": Table(context_flux_table),
