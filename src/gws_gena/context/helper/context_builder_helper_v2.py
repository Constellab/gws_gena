...
class ContextBuilderHelperV2(BaseHelper):
    """
    ...
    def build(self, net, flux_table: Table, pheno_table: Table) -> Context:
        ...
        for key, table in all_tables.items():
            ...
            if key == "rxn":
                ...
                data = net.get_reactions_dict()
            else:
                ids = table.get_column_data(entity_id_column_name)
                data = net.get_metabolites_dict()
            ...
            if isinstance(targets[0], str):  # if there is multiple simulations
                ...
                for i, ref_id in enumerate(ids):
                    if ref_id in data:
                        ...
                        ubound = ubounds[i]
                        if key == "rxn":
                            lbound = [NetworkCobra.REACTION_LOWER_BOUND if math.isnan(x) else x for x in lbound]
                            ubound = [NetworkCobra.REACTION_UPPER_BOUND if math.isnan(x) else x for x in ubound]
                        else:
                            lbound = [NetworkCobra.METABOLITE_LOWER_BOUND if math.isnan(x) else x for x in lbound]
                            ubound = [NetworkCobra.METABOLITE_UPPER_BOUND if math.isnan(x) else x for x in ubound]
                ...
                for i, ref_id in enumerate(ids):
                    if ref_id in data:
                        ...
                        if key == "rxn":
                            lbound = NetworkCobra.REACTION_LOWER_BOUND if math.isnan(lbounds[i]) else lbounds[i]
                            ubound = NetworkCobra.REACTION_UPPER_BOUND if math.isnan(ubounds[i]) else ubounds[i]
                        else :
                            lbound = NetworkCobra.METABOLITE_LOWER_BOUND if math.isnan(lbounds[i]) else lbounds[i]
                            ubound = NetworkCobra.METABOLITE_UPPER_BOUND if math.isnan(ubounds[i]) else ubounds[i]
