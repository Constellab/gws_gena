...
@task_decorator(
    "GenerationMultiSimulations",
    human_name="Generation Multi Simulations",
...
class GenerationMultiSimulations(Task):
    ...
    config_specs: ConfigSpecs = ConfigSpecs(
        {
            "number_simulations": IntParam(
                human_name="Number of simulations",
                min_value=1,
                short_description="Number of simulations to generate",
            ),
            "type_generation": StrParam(
                human_name="Distribution generation",
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        experimental_input = inputs["experimental_data"]
        if isinstance(experimental_input, Table):
            experimental_data = experimental_input.get_data()
        elif isinstance(experimental_input, File):
            experimental_input: File = inputs["experimental_data"]
            experimental_data = pd.read_csv(experimental_input.path, header=0, index_col=0, sep=",")
        ...
        simulated_data = self.create_simulated_data(
        ...
        # Generate data
        simulations_generated = self.generation_simulations_context(
            context=simulated_data,
            number_simulations=number_simulations,
            confidence_score_value=confidence_score_value,
        )

        return {"simulations": simulations_generated}

    def create_simulated_data(
        self, number_simulations: int, experimental_data: pd.DataFrame, type_generation: str
    ) -> pd.DataFrame:
        if type_generation == "Multivariate normal distribution":
            ...
            matrix_generated = np.random.default_rng().multivariate_normal(
                mean_vector, covariance_matrix, number_simulations
            )
        elif type_generation == "Normal distribution":
            mean_vector = np.mean(experimental_data, axis=1)
            std = np.std(experimental_data, axis=1)
            # Generate random numbers from the mean and the covariance
            matrix_generated = np.random.default_rng().normal(
                mean_vector, std, (number_simulations, len(experimental_data))
            )
        elif type_generation == "Uniform distribution":
            low = np.min(experimental_data, axis=1)
            high = np.max(experimental_data, axis=1)
            matrix_generated = np.random.default_rng().uniform(
                low, high, (number_simulations, len(experimental_data))
            )
        ...
        data_sim_2 = pd.DataFrame(
            {"target": matrix_generated.apply(lambda row: row.values, axis=1).tolist()}
        )
        data_sim_2.index = experimental_data.index

        return data_sim_2

    def generation_simulations_context(
        self, context: pd.DataFrame, number_simulations: int, confidence_score_value: float
    ) -> Table:
        ...
        context["lower_bound"] = context.apply(
            lambda row: [value - row["std_dev"] for value in row["target"]], axis=1
        )
        context["upper_bound"] = context.apply(
            lambda row: [value + row["std_dev"] for value in row["target"]], axis=1
        )
        ...
        context["confidence_score"] = np.array(
            [np.array2string(np.array(xi), separator=", ") for xi in context["confidence_score"]]
        )
        context["lower_bound"] = np.array(
            [np.array2string(np.array(xi), separator=", ") for xi in context["lower_bound"]]
        )
        context["upper_bound"] = np.array(
            [np.array2string(np.array(xi), separator=", ") for xi in context["upper_bound"]]
        )
        ...
        context_table: Table = Table(context)
