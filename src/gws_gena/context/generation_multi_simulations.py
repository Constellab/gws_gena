...
class GenerationMultiSimulations(Task):
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if isinstance(experimental_data, Table):
            ...
            experimental_data = pd.read_csv(
                experimental_data.path, header=0, index_col=0, sep=',')
        ...
        simulated_data = self.create_simulated_data(
            number_simulations=number_simulations, experimental_data=experimental_data, type_generation=type_generation)
        ...
        simulations_generated = self.generation_simulations_context(
            context=simulated_data, number_simulations=number_simulations, confidence_score_value=confidence_score_value)
    ...
    def create_simulated_data(self, number_simulations: int, experimental_data: pd.DataFrame, type_generation: str) -> pd.DataFrame:
        ...
        data_sim_2 = pd.DataFrame({'target': matrix_generated.apply(
            lambda row: row.values, axis=1).tolist()})
    ...
    def generation_simulations_context(self, context: pd.DataFrame, number_simulations: int, confidence_score_value: float) -> Table:
        ...
        context["confidence_score"] = context["confidence_score"].apply(
            lambda x: new_vector)

        # Convert the vector target directly into str with comma as separators and other columns to the right format
        context["target"] = context["target"].apply(
            lambda x: np.array2string(x, separator=', '))
        context["confidence_score"] = np.array([np.array2string(
            np.array(xi), separator=', ') for xi in context["confidence_score"]])
        context["lower_bound"] = np.array([np.array2string(
            np.array(xi), separator=', ') for xi in context["lower_bound"]])
        context["upper_bound"] = np.array([np.array2string(
            np.array(xi), separator=', ') for xi in context["upper_bound"]])
