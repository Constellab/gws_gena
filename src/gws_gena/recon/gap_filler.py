    ...
    *
    ...
    def _fill_gaps_with_tax(self, net, params):
        ...
        for k in dead_end_list:
            ...
            # TODO : to replace by the follwing code
            # biota_c = BiotaCompound.search_by_chebi_ids(BiotaCompound.chebi_id == comp.chebi_id)
            # if len(biota_c) == 0:
            ...
            if biota_c is None:
                self.log_warning_message(
                    f'No compound corresponds to chebi_id "{comp.chebi_id}". It is may an alternative chebi id')
                net.set_compound_recon_tag(comp.id, {
                    "id": comp.id,
                    "is_chebi_not_found": True,
                    "error": "CheBI ID not found"
