...
class TwinHelperV2:
    ...
    def create_steady_stoichiometric_matrix(cls, twin: TwinV2, ignore_cofactors=False) -> DataFrame:
        ...
        steady_metabolites: List[Metabolite] = []
        for metabolite in network.get_metabolites():
            if network.network_dict["id"] :
                name_network_with_delimiter = network.network_dict["id"]+"_"
            else:
                name_network_with_delimiter = network.network_dict["name"]+"_"
            try :
                compartment = metabolite.compartment.split(name_network_with_delimiter)[1]
            except :
                compartment = metabolite.compartment
            compartment = Compartment.get_by_bigg_id_or_go_id_or_none(compartment)
            if compartment and compartment.is_steady:
                steady_metabolites.append(metabolite)

        # filter out cofactors
        if ignore_cofactors:
            non_co_factors = []
            for metabolite in steady_metabolites:
            ...
            steady_metabolites = non_co_factors

        metabolite_ids = [metabolite.id for metabolite in steady_metabolites]
