...
@task_decorator(
    "NetworkMergem",
    human_name="Network mergem",
    short_description="Merge two networks with mergem function",
...
class NetworkMergem(Task):
    ...
    input_specs = InputSpecs(
        {
            "network_1": InputSpec(
                (File, Network),
                human_name="Network 1",
                short_description="The first network in Json or Network format",
            ),
            "network_2": InputSpec(
                (File, Network),
                human_name="Network 2",
                short_description="The second network in Json or Network format",
            ),
        }
    )
    config_specs: ConfigSpecs = ConfigSpecs(
        {
            "keep_objective": StrParam(
                default_value="merge",
                allowed_values=["merge", "1", "2"],
    ...
    output_specs = OutputSpecs(
        {
            "network": OutputSpec(
                Network, human_name="Merged network", short_description="The merged network"
            )
        }
    )
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        net_mergem = NetworkImporter.call(
            json_file,
            params={
    ...
    def export_to_json(self, network: Network):
        network = NetworkExporter.call(
            network, params={"file_name": "network", "file_format": "json"}
        )
        # Load json file
        ...
        # Save the new JSON
        with open(str(network.path), "w", encoding="utf-8") as file:
