...
from gws_core import (ConfigParams, InputSpec, InputSpecs, OutputSpec,
                      ...
                      task_decorator, File, StrParam,BoolParam, ConfigSpecs )
...
@task_decorator("NetworkMergem", human_name="Network mergem", short_description="Merge two networks with mergem function",
...
class NetworkMergem(Task):
    ...
    config_specs: ConfigSpecs = {
        "keep_objective":
        StrParam(default_value="merge", allowed_values=["merge", "1", "2"],
            human_name="Keep objective", short_description="Specifies if the objective functions are merged or copied from a single model (1 or 2)"),
        "biomass_metabolite_id_user":
        StrParam(default_value = "", human_name="Biomass metabolite id", short_description="The id of the Biomass metabolite to keep in the merged model", optional = True),
        "add_biomass":
        BoolParam(
        ...
        "trans_to_db":
        StrParam(default_value="None", allowed_values=["None", "chebi", "metacyc", "kegg", "reactome", "metanetx", "hmdb", "biocyc", "bigg", "seed", "sabiork", "rhea"],
        visibility=StrParam.PROTECTED_VISIBILITY, human_name="Translate ids", short_description="Translate metabolite and reaction IDs to a target database")
    }
    output_specs = OutputSpecs({'network': OutputSpec(Network, human_name="Merged network",
                                                      short_description="The merged network")})

    script_merge = os.path.join(os.path.abspath(os.path.dirname(__file__)),"_network_mergem.py")
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if isinstance(net1, Network):
            ...
            raise Exception("Your first model must be a Network or a Json File")
        ...
        if isinstance(net2, Network):
            ...
            raise Exception("Your second model must be a Network or a Json File")
        ...
        result = shell_proxy.run(f"python3 {self.script_merge} {net1_path} {net2_path} {keep_objective} {exact_stoichiometry} {add_annotations} {use_proton} {trans_to_db} {output_path}", shell_mode=True)
        if result != 0:
            raise Exception("An error occurred while executing the script, please check your input files or follow the error messages.")
    ...
    def export_to_json(self, network : Network) :
        ...
        # Transform the data
        data["compartments"] = {item['id']: item['name'] for item in data["compartments"]}
        # Save the new JSON
