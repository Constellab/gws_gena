...
@task_decorator(
    "Transform_metabolites_file",
    human_name="Metabolites file transformation",
    short_description="Transformation of the Bigg Models database metabolites file",
    hide=True,
)
class TransformMetabolitesFile(Task):
    ...
    output_specs = OutputSpecs(
        {
            "output": OutputSpec(
                Folder, human_name="Output folder", short_description="transformed metabolites file"
            )
        }
    )
    config_specs = ConfigSpecs(
        {
            "URL": StrParam(
                default_value="http://bigg.ucsd.edu/static/namespace/bigg_models_metabolites.txt",
                human_name="File URL",
                short_description="URL of the metabolites.txt file to download",
            )
        }
    )
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        metabolites_file = file_downloader.download_file_if_missing(
        ...
        with open(
            f"{destination_dir}/restructured_metabolites_file.txt", "w", encoding="utf-8"
        ) as new_file:
...
@task_decorator(
    "Transform_reactions_file",
    human_name="Reactions file transformation",
    short_description="Transformation of the Bigg Models database reactions file",
    hide=True,
)
class TransformReactionsFile(Task):
    ...
    output_specs = OutputSpecs(
        {
            "output": OutputSpec(
                Folder, human_name="Output folder", short_description="transformed reactions file"
            )
        }
    )
    config_specs = ConfigSpecs(
        {
            "URL": StrParam(
                default_value="http://bigg.ucsd.edu/static/namespace/bigg_models_reactions.txt",
                human_name="File URL",
                short_description="URL of the reactions.txt file to download",
            )
        }
    )
        ...
        with open(reactions_file, encoding="utf-8") as file:
            for line in file:
                ...
                if "rhea" in line:
                    ...
                    bigg_id = line.split()[0]
                    ec_number = re.findall(r"ec-code\/(\d+(?:\.\d+)*)", line)[0]
                    rhea_id = re.findall(r"rhea\/([0-9]+)", line)[0] if "rhea" in line else ""
                    reactions_dict[bigg_id] = {"rhea": str(rhea_id), "ec_number": ec_number}
        # Write the csv output
        header = ["BIGG", "RHEA", "EC-number"]
        with open(
            f"{destination_dir}/restructured_reactions_file.txt", "w", encoding="utf-8"
        ) as restructured_file:
            ...
            for key, value in reactions_dict.items():
                writer.writerow(
                    {"BIGG": key, "RHEA": value["rhea"], "EC-number": value["ec_number"]}
                )
        return {"output": Folder(destination_dir)}
