    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        with open(f"{destination_dir}/restructured_metabolites_file.txt", "w", encoding='utf-8') as new_file:
            writer = csv.DictWriter(
                new_file, fieldnames=header, delimiter='\t')
            # Write the header
            writer.writeheader()
            # Write the dictionary
            for key, value in metabolites_dict.items():
                writer.writerow(
                    {'BIGG': key, 'Name': value['name'], 'CHEBI': value['chebi']})
...
class TransformReactionsFile(Task):
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        with open(reactions_file, encoding='utf-8') as file:
            for line in file:
                ...
                if "rhea" in line:
                    ...
                    if "ec-code" in line:
                        ec_number = re.findall(
                            r"ec-code\/(\d+(?:\.\d+)*)", line)[0]
                    else:
                        ec_number = ""
                    reactions_dict[bigg_id] = {"rhea": str(
                        rhea_id), "ec_number": ec_number}
        ...
        with open(f"{destination_dir}/restructured_reactions_file.txt", "w", encoding='utf-8') as restructured_file:
            writer = csv.DictWriter(
                restructured_file, fieldnames=header, delimiter='\t')
            # Write the header
            writer.writeheader()
            # Write the dictionary
