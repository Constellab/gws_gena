...
@task_decorator(
    "ConvertAnnotation",
    human_name="Convert Model Annotation",
...
class ConvertAnnotation(Task):
    ...
    output_specs = OutputSpecs(
        {
            "output_model_annotated": OutputSpec(
                File, human_name="Model annotated", short_description="The model completed"
            ),
            "output_results": OutputSpec(
                Table,
                human_name="Results Annotation",
                short_description="The annotation result table",
            ),
        }
    )
    config_specs = ConfigSpecs(
        {
            "metabolites_id": StrParam(
                allowed_values=["", "BiGG", "Literal_name", "Other"],
                default_value=None,
                optional=False,
                human_name="Metabolite ID",
                short_description="What is the type of metabolite 'id'?",
            ),
            "metabolites_name": StrParam(
                allowed_values=["", "BiGG", "Literal_name", "Other"],
                default_value=None,
                optional=False,
                human_name="Metabolite name",
                short_description="What is the type of metabolite 'name'?",
            ),
            "reaction_id": StrParam(
                allowed_values=["", "BiGG", "Other"],
                default_value=None,
                optional=False,
                human_name="Reaction ID",
                short_description="What is the type of reaction 'id'?",
            ),
            "reaction_name": StrParam(
                allowed_values=["", "BiGG", "Other"],
                default_value=None,
                optional=False,
                human_name="Reaction name",
                short_description="What is the type of reaction 'name'?",
            ),
        }
    )

    script_conversion_annotation = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "_conversion_annotation.py"
    )
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        # check if we retrieve the output
        output_reactions_file = outputs_transform_reactions_file.get("output")
        path_reactions = output_reactions_file.list_dir_path() if output_reactions_file else []
        ...
        return {"output_model_annotated": model_annotated, "output_results": results_table}
