...
def dialog_koa_params(gena_state: State):
    ...
    st.text_input(
        translate_service.translate("koa_analysis_scenario_name"),
        placeholder=translate_service.translate("enter_koa_name"),
        value=f"{gena_state.get_current_analysis_name()} - KOA Analysis",
        key=gena_state.KOA_SCENARIO_NAME_INPUT_KEY,
    )
    ...
    st.selectbox(
        ...
        index=0,
        key=gena_state.KOA_RESOURCE_OPTION_KEY,
    )
    ...
    if gena_state.get_koa_resource_option() == translate_service.translate("create_new_resource"):
        with st.expander(
            ...
            choice_table_type = st.selectbox(
                label=translate_service.translate("please_select_option"),
                options=[
                    translate_service.translate("ec_table"),
        ...
        resource_select.select_resource(
            placeholder=translate_service.translate("search_ko_table"),
            key=gena_state.RESOURCE_SELECTOR_KO_TABLE_KEY,
        )
    ...
    with col1:
        save_clicked = st.button(
            translate_service.translate("save_koa"),
            width="stretch",
            icon=":material/save:",
            key="button_koa_save",
        )
    ...
    with col2:
        run_clicked = st.button(
            translate_service.translate("run_koa"),
            width="stretch",
    ...
    if save_clicked or run_clicked:
        if not gena_state.get_koa_config()["is_valid"] or (
            not gena_state.get_resource_selector_ko_table()
            if gena_state.get_koa_resource_option()
            == translate_service.translate("select_resource")
            else False
        ):
        ...
        with StreamlitAuthenticateUser():
            if gena_state.get_koa_resource_option() == translate_service.translate(
                "create_new_resource"
            ):
                ...
                full_path = os.path.join(
                    path_temp, f"KOTable_{gena_state.get_current_analysis_name()}.csv"
                )
            ...
            scenario = create_base_scenario_with_tags(
                gena_state,
                gena_state.TAG_KOA,
                gena_state.get_scenario_user_name(gena_state.KOA_SCENARIO_NAME_INPUT_KEY),
            )
            ...
            # Add KOA process
            koa_process = protocol.add_process(
                KOA, "koa_process", config_params=gena_state.get_koa_config()["config"]
            )
            ...
            protocol.add_connector(
                out_port=twin_resource >> "resource", in_port=koa_process << "twin"
            )
            protocol.add_connector(
                out_port=ko_table_resource >> "resource", in_port=koa_process << "ko_table"
            )

            # Add outputs
            protocol.add_output("koa_twin_output", koa_process >> "twin", flag_resource=False)
            protocol.add_output(
                "koa_result_output", koa_process >> "koa_result", flag_resource=False
            )
            protocol.add_output(
                "koa_table_summary_output", koa_process >> "table_summary", flag_resource=False
            )
...
def render_koa_step(selected_scenario: Scenario, gena_state: State) -> None:
    ...
    if not selected_scenario:
        if not gena_state.get_is_standalone():
            ...
            st.button(
                translate_service.translate("configure_new_koa"),
                icon=":material/edit:",
                width="stretch",
        ...
        twin_resource_set_dict = (
            protocol_proxy.get_process("koa_process").get_output("twin").get_resources()
        )
        network_resource, context_resource = extract_network_and_context_from_twin(
            twin_resource_set_dict
        )

        tab_flux, tab_summary, tab_network, tab_context = st.tabs(
            [
                translate_service.translate("flux_tables"),
                translate_service.translate("summary"),
        ...
        with tab_flux:
            ...
            selected_result = st.selectbox(
                translate_service.translate("select_flux_table"),
