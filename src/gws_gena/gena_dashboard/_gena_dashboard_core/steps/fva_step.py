...
def dialog_fva_params(gena_state: State):
    ...
    with col1:
        save_clicked = st.button("Save FVA", use_container_width=True, icon=":material/save:", key="button_fva_save")

    with col2:
        run_clicked = st.button("Run FVA", use_container_width=True, icon=":material/play_arrow:", key="button_fva_run")
