...
def dialog_fva_params(gena_state: State):
    ...
    st.text_input(
        translate_service.translate("fva_scenario_name"),
        placeholder=translate_service.translate("enter_fva_name"),
        value=f"{gena_state.get_current_analysis_name()} - FVA",
        key=gena_state.FVA_SCENARIO_NAME_INPUT_KEY,
    )
    ...
    with col1:
        save_clicked = st.button(
            translate_service.translate("save_fva"),
            width="stretch",
            icon=":material/save:",
            key="button_fva_save",
        )
    ...
    with col2:
        run_clicked = st.button(
            translate_service.translate("run_fva"),
            width="stretch",
    ...
    if save_clicked or run_clicked:
        ...
        with StreamlitAuthenticateUser():
            scenario = create_base_scenario_with_tags(
                gena_state,
                gena_state.TAG_FVA,
                gena_state.get_scenario_user_name(gena_state.FVA_SCENARIO_NAME_INPUT_KEY),
            )
            protocol = scenario.get_protocol()

            # Add FVA process
            fva_process = protocol.add_process(
                FVA, "fva_process", config_params=gena_state.get_fva_config()["config"]
            )
            ...
            protocol.add_connector(
                out_port=twin_resource >> "resource", in_port=fva_process << "twin"
            )

            # Add outputs
            protocol.add_output("fva_twin_output", fva_process >> "twin", flag_resource=False)
            protocol.add_output(
                "fva_result_output", fva_process >> "fva_result", flag_resource=False
            )
...
def render_fva_step(selected_scenario: Scenario, gena_state: State) -> None:
    ...
    if not selected_scenario:
        if not gena_state.get_is_standalone():
            ...
            st.button(
                translate_service.translate("configure_new_fva"),
                icon=":material/edit:",
                width="stretch",
        ...
        fva_result = (
            protocol_proxy.get_process("fva_process").get_output("fva_result").get_resources()
        )

        twin_resource_set_dict = (
            protocol_proxy.get_process("fva_process").get_output("twin").get_resources()
        )
        ...
        with tab_flux:
            ...
            for idx in flux_data.index:
                if isinstance(idx, str) and "_simu" in idx:
            ...
            if simulation_numbers:
                simulation_numbers = sorted(simulation_numbers)
            ...
            selected_reactions = st.multiselect(
                ...
                default=reaction_names[
                    : min(10, len(reaction_names))
                ],  # Default to first 10 reactions
            )
            ...
            if selected_reactions:
                ...
                fig_range = px.scatter(
                    ...
                    labels={
                        "Reaction": translate_service.translate("reaction_names"),
                        "Value": translate_service.translate("flux_values"),
                    },
                    hover_data=["Lower_Bound", "Upper_Bound", "Range"],
                )
                fig_range.update_traces(marker={"size": 8})
