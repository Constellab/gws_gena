...
from gws_streamlit_main import StreamlitContainers, StreamlitResourceSelect, StreamlitTaskRunner
...
def _create_empty_context_resource(gena_state: State) -> ResourceModel:
    ...
    full_path = os.path.join(
        path_temp, f"{gena_state.get_current_analysis_name()}_empty_context.json"
    )
    ...
    return ResourceModel.save_from_resource(
        context_file, origin=ResourceOrigin.UPLOADED, flagged=True
    )
    ...
    if translate_service.translate("experimental_data_metabolite") in simulation_data_types:
        ...
        if metabolite_data:
            ...
            metabolite_generation = protocol.add_process(
                GenerationMultiSimulations, "metabolite_generation_process", metabolite_config
            )
            protocol.add_connector(
                metabolite_input >> "resource", metabolite_generation << "experimental_data"
            )
            protocol.add_connector(
                metabolite_generation >> "simulations", context_process << "pheno_table"
            )
    ...
    if translate_service.translate("experimental_data_reaction") in simulation_data_types:
        ...
        if reaction_data:
            ...
            reaction_generation = protocol.add_process(
                GenerationMultiSimulations, "reaction_generation_process", reaction_config
            )
            protocol.add_connector(
                reaction_input >> "resource", reaction_generation << "experimental_data"
            )
            protocol.add_connector(
                reaction_generation >> "simulations", context_process << "flux_table"
            )
...
def _handle_existing_context(protocol: ProtocolProxy, gena_state: State) -> bool:
    ...
    if selected_context.resource_typing_name == "RESOURCE.gws_gena.Context":
        ...
        protocol.add_output(
            "context_process_output", context_process >> "resource", flag_resource=False
        )
        return True
...
def _handle_context_deg_builder(protocol: ProtocolProxy, gena_state: State) -> None:
    ...
    context_deg = protocol.add_process(ContextFromDEG, "context_deg_process", deg_config)

    deg_data = gena_state.get_resource_selector_deg_data()

    if deg_data:
        deg_input = protocol.add_process(
            InputTask, "deg_data_resource", {InputTask.config_name: deg_data["resourceId"]}
        )
        if gena_state.get_gene_translation_selector():
            ...
            context_convert = protocol.add_process(
                IDConvertTask, "id_convert_process", convert_config
            )
...
def _render_context_setup_ui(gena_state: State) -> bool:
    ...
    url_doc_context = (
        "https://constellab.community/bricks/gws_gena/latest/doc/technical-folder/resource/Context"
    )
    ...
    col_question, col_help = StreamlitContainers.columns_with_fit_content(
        "container-column_context", cols=[1, "fit-content"], vertical_align_items="center"
    )
    with col_question:
        st.selectbox(
    ...
    if context_choice == translate_service.translate("no"):
        ...
        st.selectbox(
            ...
            options=[
                translate_service.translate("select_existing_context"),
                translate_service.translate("build_new_context"),
        ...
        if context_option == translate_service.translate("select_existing_context"):
            col_select, col_help = StreamlitContainers.columns_with_fit_content(
                "container-column_context_importer",
                cols=[1, "fit-content"],
            ...
            col_info, col_help = StreamlitContainers.columns_with_fit_content(
                "container-column_context_simulation",
                cols=[1, "fit-content"],
                vertical_align_items="center",
            )
            with col_info:
            ...
            selected_data_types = st.multiselect(
                ...
                options=[
                    translate_service.translate("experimental_data_metabolite"),
                    translate_service.translate("experimental_data_reaction"),
                ],
            ...
            if translate_service.translate("experimental_data_metabolite") in selected_data_types:
                ...
                metabolite_resource_valid = (
                    gena_state.get_resource_selector_metabolite_data() is not None
                )
                ...
                if metabolite_resource_valid:
                    ...
                    metabolite_config = (
                        gena_state.get_generation_multi_simulations_metabolite_config()
                    )
                    metabolite_config_valid = (
                        metabolite_config.get("is_valid", False) if metabolite_config else False
                    )
            ...
            if translate_service.translate("experimental_data_reaction") in selected_data_types:
                ...
                reaction_resource_valid = (
                    gena_state.get_resource_selector_reaction_data() is not None
                )
                ...
                if reaction_resource_valid:
                    ...
                    reaction_config = gena_state.get_generation_multi_simulations_reaction_config()
                    reaction_config_valid = (
                        reaction_config.get("is_valid", False) if reaction_config else False
                    )
            ...
            if deg_resource_valid:
                ...
                if gena_state.get_gene_translation_selector():
                    ...
                    metabolite_config_valid = (
                        metabolite_convert_config.get("is_valid", False)
                        if metabolite_convert_config
                        else False
                    )
                    all_widgets_valid.append(metabolite_config_valid)
            ...
            col_info, col_help = StreamlitContainers.columns_with_fit_content(
                "container-column_context_builder",
                cols=[1, "fit-content"],
                vertical_align_items="center",
            )
            with col_info:
            ...
            return (
                ...
                or gena_state.get_resource_selector_flux()
            )
        else:
...
def _render_context_creation_ui(gena_state: State) -> None:
    ...
    if st.button(
        translate_service.translate("run_context_importer"),
        ...
        if context_choice == translate_service.translate("no"):
            ...
            if context_option == translate_service.translate("build_new_context"):
                ...
                metabolite_valid = (
                    translate_service.translate("experimental_data_metabolite")
                    in selected_data_types
                    and gena_state.get_resource_selector_metabolite_data()
                )
                reaction_valid = (
                    translate_service.translate("experimental_data_reaction") in selected_data_types
                    and gena_state.get_resource_selector_reaction_data()
                )
                has_required_resources = metabolite_valid or reaction_valid
            elif context_option == translate_service.translate("build_new_context_deg"):
                if gena_state.get_gene_translation_selector():
                    has_required_resources = bool(
                        gena_state.get_resource_selector_deg_data()
                        and gena_state.get_id_convert_config()
                        and gena_state.get_context_from_deg_config()
                    )
                else:
                    has_required_resources = bool(
                        gena_state.get_resource_selector_deg_data()
