...
def render_new_analysis_page(gena_state : State):
    ...
    with st.form(clear_on_submit=False, enter_to_submit=True, key="new_analysis_form"):
        ...
        resource_select.select_resource(
            placeholder='Search for network resource', key=gena_state.RESOURCE_SELECTOR_NETWORK_KEY, defaut_resource=None)

        form_config = StreamlitTaskRunner(NetworkImporter)
        form_config.generate_config_form_without_run(
            session_state_key=gena_state.NETWORK_IMPORTER_CONFIG_KEY, default_config_values=NetworkImporter.config_specs.get_default_values())
        ...
        if submit_button:
            ...
            name_network = selected_network.get_resource().get_name()

            # We parse value to ensure it is a valid tag format because auto parse is not longer availaible
            # for values in lab
            name_network_parsed = Tag.parse_tag(name_network)
            analysis_name_parsed = Tag.parse_tag(analysis_name)

            # Add tags to the scenario
            scenario.add_tag(Tag(gena_state.TAG_NETWORK, name_network_parsed, is_propagable=False, auto_parse=True))
            ...
            # Add output
            protocol.add_output('network_importer_process_output', network_importer_process >> 'target', flag_resource=False)
