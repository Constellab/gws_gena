...
def render_scenario_table(
    scenarios: list[Scenario], process_name: str, grid_key: str, gena_state: State
) -> None:
...
def create_base_scenario_with_tags(gena_state: State, step_tag: str, title: str) -> ScenarioProxy:
    ...
    scenario.add_tag(
        Tag(
            ...
            gena_state.get_current_analysis_name(),
            is_propagable=False,
            auto_parse=True,
        )
    )
    scenario.add_tag(
        Tag(
            ...
            gena_state.get_current_gena_pipeline_id(),
            is_propagable=False,
            auto_parse=True,
        )
    )
...
def add_tags_on_network(edited_network: Network, gena_state: State) -> None:
    ...
    entity_tags.add_tag(
        Tag(gena_state.TAG_GENA, gena_state.TAG_NETWORK_UPDATED, is_propagable=False)
    )
    entity_tags.add_tag(
        Tag(
            gena_state.TAG_GENA_PIPELINE_ID,
...
def display_network(network_model_id: str) -> None:
    ...
    generate_link_dto = GenerateShareLinkDTO.get_1_hour_validity(
        entity_id=network_model_id, entity_type=ShareLinkEntityType.RESOURCE
    )
...
def build_scenarios_by_step_dict(
    gena_pipeline_id: str, gena_state: State
    ...
    search_scenario_builder = (
        ScenarioSearchBuilder()
        .add_tag_filter(
            Tag(key=gena_state.TAG_GENA_PIPELINE_ID, value=gena_pipeline_id_parsed, auto_parse=True)
        )
