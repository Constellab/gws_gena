...
class ReactionAdderHelper(BaseHelper):
    ...
    def add_reactions(
        ...
        if reaction_table.column_exists(ec_number_name):
            ...
            raise Exception(
                f"Cannot import reaction table : no column with name '{ec_number_name}' found or '{id_column_name}', use the Transformer EC Number Table or Transformer Entity ID Table"
            )
