...
class ReactionRemover(Task):
    input_specs = InputSpecs(
        {
            "network": InputSpec(
                Network, human_name="Network", short_description="Network to trim"
            ),
            "reaction_table": InputSpec(
                Table,
                human_name="Table of EC numbers or IDs",
                short_description="The table of reactions to remove",
            ),
        }
    )
    output_specs = OutputSpecs(
        {
            "network": OutputSpec(
                Network, human_name="Network", short_description="The network after trimming"
            )
        }
    )
    config_specs = ConfigSpecs(
        {
            "reverse_remove": BoolParam(
                default_value=False,
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        helper.remove_list_of_reactions(network, reaction_table, reverse_remove=reverse_remove)
