...
from gws_core import (ConfigParams, InputSpec, OutputSpec, File,
                      Task, TaskInputs, TaskOutputs, task_decorator, StrParam, FloatParam, PlotlyResource, Table, BoolParam)
import pandas as pd
import numpy as np
...
class PlotFluxTableAnalysis(Task):
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        for i in range (0, len(flux_condition1["value"])-1):
            ...
            row = pd.DataFrame([[tag,reaction_id,flux_condition1_value,flux_condition2_value]], columns=columns_changements)
        ...
        # Scatter plot
        target = px.scatter(source, x= "flux_" + name_condition1 , y= "flux_" + name_condition2, color='Tag', log_x=log_x, log_y=log_y)
