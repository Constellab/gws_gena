...
@task_decorator(
    "FBA",
    human_name="FBA",
    short_description="Flux balance analysis",
...
class FBA(Task):
    ...
    output_specs = OutputSpecs(
        {
            ...
            "fba_result": OutputSpec(
                FBAResult, human_name="FBA result tables", short_description="The FBA result tables"
            ),
        }
    )
    config_specs = ConfigSpecs(
        {
            "biomass_optimization": SelectParam(
                options=["", "maximize", "minimize"],
                default_value=None,
                optional=True,
                human_name="Biomass optimization",
                short_description="Biomass optimization",
            ),
            "fluxes_to_maximize": ListParam(
                default_value=None,
                optional=True,
                visibility=StrParam.PROTECTED_VISIBILITY,
                human_name="Fluxes to maximize",
                short_description="The fluxes to maximize",
            ),
            "fluxes_to_minimize": ListParam(
                default_value=None,
                optional=True,
                visibility=StrParam.PROTECTED_VISIBILITY,
                human_name="Fluxes to minimize",
                short_description="The fluxes to minimize",
            ),
            "solver": SelectParam(
                default_value="quad",
                visibility=StrParam.PROTECTED_VISIBILITY,
                options=["quad", "highs-ds", "highs-ipm", "highs", "interior-point"],
                human_name="Solver",
                short_description="The optimization solver. It is recommended to use `quad`. Other solvers are in `beta` versions.",
            ),
            "relax_qssa": BoolParam(
                default_value=False,
                visibility=StrParam.PROTECTED_VISIBILITY,
                human_name="Relax QSSA",
                short_description="True to relaxing the quasi-steady state assumption (QSSA) constrain (`quad` solver is used). False otherwise.",
            ),
            "qssa_relaxation_strength": FloatParam(
                default_value=0.0,
                min_value=0.0,
                visibility=StrParam.PROTECTED_VISIBILITY,
            ...
            "number_of_simulations": IntParam(
                default_value=None,
                min_value=1,
                optional=True,
                visibility=StrParam.PROTECTED_VISIBILITY,
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        return {"fba_result": merged_fba_result, "twin": result_twin}
