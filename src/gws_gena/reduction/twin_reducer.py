...
class TwinReducer(Task):
    ...
    input_specs = InputSpecs(
        {
            "twin": InputSpec(
                (Twin, FlatTwin),
                human_name="Digital twin",
                short_description="The digital twin to reduce",
            ),
        }
    )
    output_specs = OutputSpecs(
        {
            "efm_table": OutputSpec(
                TwinEFMTable,
                human_name="EFM table",
                short_description="The table of elementary flux modes",
            ),
            "reduction_table": OutputSpec(
                TwinReductionTable,
                human_name="Reduction table",
                short_description="The reduction table",
            ),
        }
    )
    config_specs = ConfigSpecs(
        {
            "ignore_cofactors": BoolParam(
                default_value=False,
                human_name="Ignore cofactors",
                short_description="Set True to ignore cofactors, False otherwise.",
            ),
            "reversible_reactions": ListParam(
                default_value=None,
                optional=True,
                human_name="List of reversible reaction",
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        reduced_matrices = TwinHelper.compute_reduced_matrices(
            flat_twin, ignore_cofactors=params.get_value("ignore_cofactors"), reversibilities=rev
        )

        twin_efm_table = TwinEFMTable(data=reduced_matrices["EFM"])
        twin_reduction_table = TwinReductionTable(data=reduced_matrices["K"])

        return {"efm_table": twin_efm_table, "reduction_table": twin_reduction_table}
