...
@task_decorator(
    "GapFiller",
    human_name="Gap filler",
    short_description="Fills gaps in networks",
...
class GapFiller(Task):
    ...
    input_specs = InputSpecs(
        {
            "network": InputSpec(
                Network, human_name="Network", short_description="The network to gap-fill"
            )
        }
    )
    ...
    output_specs = OutputSpecs(
        {
            "network": OutputSpec(
                Network, human_name="Network", short_description="The gap-filled network"
            )
        }
    )
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        net_output = helper.fill_gaps(net)

        return {"network": net_output}
