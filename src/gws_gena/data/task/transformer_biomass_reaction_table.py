...
@task_decorator("TransformerBiomassReactionTable", human_name="Transformer Biomass Reaction",
...
class TransformerBiomassReactionTable(Task):
    ...
    * The next columns are:
    ...
    config_specs: ConfigSpecs = {
        ...
        'biomass_column':
        StrParam(
            default_value=biomass_column_name, human_name="Biomass column name",
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if not table.column_exists(name_entity_column):
            raise Exception(f"Cannot import BiomassReactionTable.  No entity column found (no column with name '{name_entity_column}')")
        if not table.column_exists(name_chebi_id_column):
            raise Exception(f"Cannot import BiomassReactionTable. No CheBI ID column found (no column with name '{name_chebi_id_column}')")
        if not table.column_exists(name_biomass_column):
            raise Exception(f"Cannot import BiomassReactionTable.  No biomass equation found (no column with name '{name_biomass_column}')")
        ...
        if name_chebi_id_column != self.chebi_id_column_name:
            table.set_column_name(current_name = name_chebi_id_column,new_name = self.chebi_id_column_name)
        if name_biomass_column!= self.biomass_column_name:
            table.set_column_name(current_name = name_biomass_column,new_name = self.biomass_column_name)
