...
class BioModel(Resource):
    ...
    def __init__( self, *args, **kwargs ):
        ...
        if self.id:
            self.__build_from_dump(self.data)
        else:
            self.data = {
                'name': '',
                'description': '',
    ...
    def __build_from_dump(self, data):
        ...
        for val in data.get("networks",[]):
            uri = val["uri"]
            nets[uri] = Network.get(Network.uri == uri)
            self.add_network(nets[uri])
        for val in data.get("contexts",[]):
            uri = val["uri"]
            ctx = Context.get(Context.uri == uri)
            for net_ctx in data.get("network_contexts",[]):
                ctx_uri = net_ctx["context_uri"]
                if ctx_uri == uri:
                    net_uri = net_ctx["network_uri"]
                    self.add_context(ctx, related_network=nets[net_uri])
                    break
    ...
    def from_json(cls, data: dict):
        bio = BioModel()
        bio.__build_from_dump(data)
        bio.data["biomodel"] = bio.dumps()
        return bio
    ...
    def save(self, *args, **kwargs):
        ...
        for ctx in self.contexts.values():
            ctx.save() 
        self.data["biomodel"] = self.dumps() #update biomodel
...
class FlatBioModel(BioModel):
    ...
    def __init__(self, flatten_json: dict = None):
        super().__init__()
        if not self.id:
            bio_dict: dict = flatten_json["biomodel"]
            net = Network.from_json(bio_dict["networks"][0])
            ctx = Context.from_json(bio_dict["contexts"][0])
            self.add_network(net, related_context=ctx)
            self.data["name"] = flatten_json["name"]
            self.data["description"] = flatten_json["description"]
            self.data["mapping"] = flatten_json["mapping"]
            self.data["reverse_mapping"] = flatten_json["reverse_mapping"]
            self.data["is_flat"] = True
