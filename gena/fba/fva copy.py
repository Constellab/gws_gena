...
def _do_parallel_scipy_loop(kwargs):
...
#         xmax = x0[i]
#     else:
#         refinement_bounds = deepcopy(bounds)
#         for k in indexes_of_fluxes_to_minimize:
#             refinement_bounds[k][0] = x0[k]*0.9
#             refinement_bounds[k][1] = x0[k]*1.1
#             #print(f"index={i}, {c.index[i]}")
#             #cf.iloc[k,0] = 2.0
...
#         cf.iloc[i,0] = 1.0
#         if solver == "quad":
#             res_fva: OptimizeResult = FBA.solve_cvxpy( 
#                 cf, A_eq, b_eq, bounds,
...
class FVA(Process):
    ...
    config_specs = {
        ...
        "solver": {"type": str, "default": "highs", "allowed_values": ["quad", "highs-ds", "highs-ipm", "highs", "interior-point"], "Description": "The optimization solver"},
        "fill_gaps_with_sinks": {"type": bool, "default": False, "Description": "True to fill gaps using sink reaction. False otherwise"},
        "relax_qssa": {"type": bool, "default": False, "Description": "True to relaxing the quasi-steady state constrain. False otherwise."}
    }
    ...
    async def task(self):
        ...
        relax_qssa = self.get_param("relax_qssa")
        ...
        if relax_qssa and solver != "quad":
            self.progress_bar.add_message(message=f"Change solver to '{solver}' for constrain relaxation.")
            solver = "quad"
        ...
        self.progress_bar.add_message(message=f"Starting optimization with solver '{solver}' ...")
        if solver == "quad":
        ...
        self.progress_bar.add_message(message=f"Peforming variability analysis around the optimal value using solver '{solver}' ...")
        x0 = res.x
        m = x0.shape[0]
        step = max(1, int(m/10)) # plot only 10 iterations on screen

        if solver == "quad":
            xmin, xmax = self.__solve_with_cvxpy(   warm_solver, 
                                                    c, A_eq, b_eq, bounds, x0,
                                                    fluxes_to_maximize,
                                                    fluxes_to_minimize,
                                                    step, m, solver, relax_qssa)
        else:
            xmin, xmax = self.__solve_with_scipy(   c, A_eq, b_eq, bounds, x0,
                                                    fluxes_to_maximize,
                                                    fluxes_to_minimize,
                                                    step, m, solver, relax_qssa)
        res.xmin = xmin
        res.xmax = xmax
        result = FVAResult(biomodel=bio, optimize_result=res)
        self.output["result"] = result

    @staticmethod
    def __solve_with_scipy( c, A_eq, b_eq, bounds, x0,
                            fluxes_to_maximize,
                            fluxes_to_minimize,
                            step, m, solver, relax_qssa):
        ...
        result = pool.map(_do_parallel_scipy_loop, params)
        # gather results
    ...
    @staticmethod
    def __solve_with_cvxpy( warm_solver, 
                            c, A_eq, b_eq, bounds, x0,
                            fluxes_to_maximize,
                            fluxes_to_minimize,
                            step, m, solver, relax_qssa):
