...
from ..biomodel.biomodel_service import BioModelService
...
class FBA(Process):
    ...
    config_specs = {
        ...
        "solver": {"type": str, "default": "quad", "allowed_values": ["highs-ds", "highs-ipm", "highs", "interior-point", "quad"], "Description": "The optimization solver"},
    ...
    async def task(self):
        ...
        self.progress_bar.set_value(2, message=f"Starting optimization with solver '{solver}' ...")
        if solver == "quad":
        ...
        result = FBAResult(biomodel=bio, optimize_result = res)
    ...
    def build_problem( cls, bio, *, fluxes_to_maximize = [], fluxes_to_minimize = [], fill_gaps_with_sinks=True):
        flat_bio: FlatBioModel = FlatBioModel(bio.flatten())
