...
from gws_gena import (EntityIDTable, EntityIDTableImporter, KnockOutAnalysis,
...
class TestFba(BaseTestCaseUsingFullBiotaDB):
    ...
    async def test_toy_koa(self):
        ...
        net = NetworkImporter.call(
            File(path=os.path.join(data_dir, "ko_analysis", "toy", "toy_ko.json")),
            {}
        )
        ctx = TwinContextImporter.call(
            File(path=os.path.join(data_dir, "ko_analysis", "toy", "toy_ko_context.json")),
            {}
        )
        ko_table = EntityIDTableImporter.call(
            File(path=os.path.join(data_dir, "ko_analysis", "toy", "ko_table.csv")),
            {}
        )
    ...
    async def test_ecoli_koa(self):
        ...
        ko_table = EntityIDTableImporter.call(
            File(path=os.path.join(data_dir, "ko_analysis", "ecoli", "ko_table.csv")),
            {}
        )
