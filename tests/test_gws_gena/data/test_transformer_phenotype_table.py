...
from gws_gena import TransformerPhenotypeTable
...
class TestTransformerPhenotypeTable(BaseTestCaseLight):
    ...
    def test_transformer_phenotype_table_(self):
        ...
        phenotype_table = TableImporter.call(File(path=os.path.join(data_dir, "data/phenotype_table_transformer_bad_names.csv")), params={"index_column": -1})

        # run trainer
        tester = TaskRunner(
            inputs={"table": phenotype_table},
            task_type=TransformerPhenotypeTable,
            params = {'entity_id_column': "id_name",
        ...
        self.assertEqual(phenotype_table.get_column_names(), ['id', 'target', 'lower_bound', 'upper_bound', 'confidence_score', 'chebi_id'])
