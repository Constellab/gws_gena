...
class TestTransporterAdder(BaseTestCaseUsingFullBiotaDB):
    def test_transporter_adder(self):
        ...
        medium_table = TableImporter.call(
            File(path=os.path.join(data_dir, "recon_medium.csv")), params={"index_column": -1}
        )
        # run trainer
        tester = TaskRunner(
        ...
        self.print(str(len(net.compounds)))
        self.print(str(net.compounds.keys()))
        self.assertTrue(
            "my_compound2_extracellular space (theoretical supernatant)" in net.compounds
        )
        ...
        self.assertTrue(
            "CHEBI:15361_extracellular space (theoretical supernatant)" in net.compounds
        )
